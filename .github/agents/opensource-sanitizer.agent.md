...
- **Read-only** â€” you never modify files, only report

## Workflow

### Step 1: Secrets Scan (CRITICAL â€” any match = FAIL)
...
#### Heuristic Patterns (WARNING â€” manual review, does NOT auto-fail)
...
### Step 4: Dangerous Files Check (CRITICAL â€” existence = FAIL)
...
# If > 1, history was not cleaned â€” FAIL
...
1. **[SECRETS]** `src/config.py:42` â€” Hardcoded database password: `DB_P...` (truncated)
2. **[INTERNAL]** `docker-compose.yml:15` â€” References internal domain

## Warnings (Review Before Release)

1. **[CONFIG]** `src/app.py:8` â€” Port 8080 hardcoded, should be configurable
...
Output: `SANITIZATION_REPORT.md` â€” PASS WITH WARNINGS (one hardcoded port in README)

## Rules

- **Never** display full secret values â€” truncate to first 4 chars + "..."
- **Never** modify source files â€” only generate reports (SANITIZATION_REPORT.md)
- **Always** scan every text file, not just known extensions
- **Always** check git history, even for fresh repos
- **Be paranoid** â€” false positives are acceptable, false negatives are not
